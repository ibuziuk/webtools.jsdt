...
import org.eclipse.wst.jsdt.chromium.debug.js.FrontEndDebugerPlugin;
...
public class ChromiumRunner implements IJSRunner {
	...
	public IProcess run(JSRunnerConfiguration configuration, ILaunch launch, IProgressMonitor monitor)
		...
		try {
			...
			return DebugPlugin.newProcess(launch, process, "Chrome / Chromiun process", //$NON-NLS-1$
					LaunchConfigurationUtil.getDefaultAttributes());
		} catch (Exception ex) {
			throw new CoreException(new Status(IStatus.ERROR, FrontEndDebugerPlugin.PLUGIN_ID,
